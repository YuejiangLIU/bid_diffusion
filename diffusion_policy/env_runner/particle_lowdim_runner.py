...
import pdb
from diffusion_policy.sampler.single import consensus_sampler, coherence_sampler
from diffusion_policy.sampler.multi import contrastive_sampler

class ParticleLowdimRunner(BaseLowdimRunner):
    def __init__(self,
            output_dir,
            n_train=10,
            n_train_vis=3,
    ...
    def set_sampler(self, sampler, nsample=1, topk=1, noise=0.0, decay=1.0):
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            pbar = tqdm.tqdm(total=self.max_steps, desc=f"Eval ParticleLowdimRunner {chunk_idx+1}/{n_chunks}", 
            ...
            while not done:
                ...
                with torch.no_grad():
                    ...
                    elif self.sampler == 'consensus':
                        action_dict = consensus_sampler(policy, obs_dict, self.n_samples, self.topk)
        ...
        total_steps_till_success = collections.defaultdict(list)
        ...
        for i in range(n_inits):
            ...
            p1 = total_reward > 0.4
            p2 = total_reward > 0.9
            steps_till_success = len(this_rewards) if total_reward > 0.9 else self.max_steps

            total_rewards[prefix].append(total_reward)
            total_p1[prefix].append(p1)
            total_p2[prefix].append(p2)
            total_steps_till_success[prefix].append(steps_till_success)
        ...
        for prefix, value in total_p2.items():
            ...
            value = np.mean(value)
            log_data[name] = value
        for prefix, value in total_steps_till_success.items():
            name = prefix+'mean_steps_till_success'
            value = np.mean(value)
            log_data[name] = value
